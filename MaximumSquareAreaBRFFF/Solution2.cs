...
public class Solution2
{
    public int MaximizeSquareArea(int m, int n, int[] hFences, int[] vFences)
    {
        ...
        for (int i = 0; i < hSums.Length; i++)
        {
            ...
            {
                long? cResult = Solve(i, hSums.Length - 1, j, vSums.Length - 1, hSums, vSums);
                if ( cResult.HasValue)
                {
    ...
    private long? Solve(int hIndex1, int hIndex2, int vIndex1, int vIndex2, long[] hSums, long[] vSums)
    {
        ...
        while (true)
        {
            ...
            hSmallSum = hIndex1 > 0 ? hSums[hIndex1 - 1] : 0;
            hSideLength = hSums[hIndex2] - hSmallSum;

            vSmallSum = vIndex1 > 0 ? vSums[vIndex1 - 1] : 0;
            vSideLength = vSums[vIndex2] - vSmallSum;
        }
    }
