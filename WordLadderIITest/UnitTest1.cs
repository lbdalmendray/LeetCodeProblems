...
namespace WordLadderIITest
{
    ...
    public class UnitTest1
    {
        ...
        public void TestMethod1()
        {
            ...
            string endWord = "cog";
            var results = solution.FindLadders(beginWord/*"hit"*/, endWord/*"cog"*/, new string[] { "hot", "dot", "dog", "lot", "log", "cog" }).ToArray();
            ...
            {
                Assert.IsTrue(results.Any(result => AreEquivalent(result, path)));
            }

            for (int i = 0; i < results.Length; i++)
            {
                var rest = results.Where((e, j) => i != j);
                Assert.IsTrue(rest.All(r=>!AreEquivalent(r,results[i])));
            }

            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
        ...
        {
            Solution solution = new Solution();
            string beginWord = "amara";
            string endWord = "amaee";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "amaaa" , "amaae", "amaee","amaea" }).ToArray();
            var paths = new string[][]
            {
                new string[] { "amara", "amaea", "amaee" }
            };
            foreach (var path in paths)
            {
                Assert.IsTrue(results.Any(result => AreEquivalent(result, path)));
        ...
        public void TestMethod3()
        {
            Solution solution = new Solution();
            string beginWord = "amara";
            string endWord = "amazz";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "amaaa", "amazz" }).ToArray();
        ...
        public void TestMethod4()
        {
            ...
            string beginWord = "amara";
            string endWord = "amazz";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "amazz" }).ToArray();
            Assert.IsTrue(results.Length == 0);
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
        }
        ...
        public void TestMethod5()
        {
            ...
            string beginWord = "hit";
            string endWord = "cog";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "hot", "dot", "dog", "lot", "log" }).ToArray();
            Assert.IsTrue(results.Length == 0);
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
        ...
        public void TestMethod6()
        {
            ...
            string beginWord = "a";
            string endWord = "c";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "a", "b", "c" }).ToArray();
            var paths = new string[][]
            {
                    new string[] { "a",  "c" }
            };
        ...
        public void TestMethod7()
        {
            ...
            string beginWord = "qa";
            string endWord = "sq";
            var results = solution.FindLadders("qa", "sq", new string[] { "si", "go", "se", "cm", "so", "ph", "mt", "db", "mb", "sb", "kr", "ln", "tm", "le", "av", "sm", "ar", "ci", "ca", "br", "ti", "ba", "to", "ra", "fa", "yo", "ow", "sn", "ya", "cr", "po", "fe", "ho", "ma", "re", "or", "rn", "au", "ur", "rh", "sr", "tc", "lt", "lo", "as", "fr", "nb", "yb", "if", "pb", "ge", "th", "pm", "rb", "sh", "co", "ga", "li", "ha", "hz", "no", "bi", "di", "hi", "qa", "pi", "os", "uh", "wm", "an", "me", "mo", "na", "la", "st", "er", "sc", "ne", "mn", "mi", "am", "ex", "pt", "io", "be", "fm", "ta", "tb", "ni", "mr", "pa", "he", "lr", "sq", "ye" }).ToArray();
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
            Assert.IsTrue(results.Length == 51);

        }      
        bool IsSolution(string beginWord , string endWord, IList<string> values)
        {
            if (!(values[0] == beginWord && values.Last() == endWord))
                return false;
            for (int i = 0; i < values.Count-1; i++)
            {
                if (!AreNearByLetter(values[i], values[i + 1],out int diffindex))
                    return false;
            }
        ...
        public static bool AreNearByLetter(string info1, string info2, out int diffIndex)
        {
            int differences = 0;
            diffIndex = -1;
        ...
        public void TestMethod9()
        {
            ...
            string beginWord = "aa";
            string endWord = "zz";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "aa" , "ba", "ca", "da", "ea" ,"bb", "cc", "dd", "ee", "bz", "cz", "dz", "ez", "zz" }).ToArray();
        ...
        public void TestMethod11()
        {
            ...
            string beginWord = "a";
            string endWord = "b";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "b" }).ToArray();
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
        ...
        public void TestMethod12()
        {
            ...
            string endWord = "bbbb";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "aaab" ,"aabb" , "abbb" , "bbbb" }).ToArray();
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
            Assert.IsTrue(results.Length == 1);
        ...
        public void TestMethod13()
        {
            ...
            string endWord = "bbbbb";
            var results = solution.FindLadders(beginWord, endWord, new string[] { "aaaab", "aaabb", "aabbb", "abbbb" ,"bbbbb" }).ToArray();
            Assert.IsTrue(results.All(sol => IsSolution(beginWord, endWord, sol)));
            Assert.IsTrue(results.Length == 1);
...
["si","go","se","cm","so","ph","mt","db","mb","sb","kr","ln","tm","le","av","sm","ar","ci","ca","br","ti","ba","to","ra","fa","yo","ow","sn","ya","cr","po","fe","ho","ma","re","or","rn","au","ur","rh","sr","tc","lt","lo","as","fr","nb","yb","if","pb","ge","th","pm","rb","sh","co","ga","li","ha","hz","no","bi","di","hi","qa","pi","os","uh","wm","an","me","mo","na","la","st","er","sc","ne","mn","mi","am","ex","pt","io","be","fm","ta","tb","ni","mr","pa","he","lr","sq","ye"]
    ...
    */
        ...
        private bool AreEquivalent(IEnumerable<string> list1, IEnumerable<string> list2)
        {
            var listArra1 = list1.ToArray();
            var listArra2 = list2.ToArray();

            if (listArra1.Length != listArra2.Length)
                return false;

            for (int i = 0; i < listArra1.Length; i++)
            {
                if (listArra1[i] != listArra2[i])
                    return false;
            }
